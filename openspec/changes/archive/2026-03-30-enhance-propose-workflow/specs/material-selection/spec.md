...
The `mysd:propose` skill SHALL detect the following requirement sources during the material selection step:
...
The `mysd:propose` skill SHALL present all detected sources as a numbered list and allow the user to select one or more sources.
...
When `auto_mode` is true, the `mysd:propose` skill SHALL automatically aggregate content from all detected sources without presenting a selection prompt.
