...
## Requirement: Proposal Type Field

Proposal frontmatter MUST include a `type` field indicating Feature/BugFix/Refactor.

The `type` field MUST accept exactly one of the following values: `Feature`, `BugFix`, `Refactor`.

### Scenario: Proposal with Type Field

WHEN a user runs `mysd propose my-feature --type Feature`
THEN the generated `proposal.md` frontmatter includes `type: Feature`

### Scenario: Missing Type Field Rejected

WHEN a proposal is created without specifying a `type` value
THEN the system MUST reject the proposal with an error indicating the `type` field is required
