...
- `instruction`: Dynamic guidance based on current state (task progress, preflight results)
...
## Step 2c: Read Instruction

Read the `instruction` field from the JSON output. This field is dynamically generated by the binary and contains context-aware guidance based on the current task state and preflight results.
